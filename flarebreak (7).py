...
_LIVE_STATE = {"data": None, "using_real_data": True, "model": None,
               "feature_cols": None, "model_accuracy": None,
...
# Converts a raw flux value into a small whole number: which flare band it's in
# (0=A, 1=B, 2=C, 3=M, 4=X) - this lets us measure "did it move UP a class?"
def flare_band_index(flux):
    # Goes through the band edges from strongest to weakest
    for i in range(len(BAND_EDGES) - 2, -1, -1):
        # If the flux is at least as big as this band's lower edge...
        if flux >= BAND_EDGES[i]:
            # Return which band index this is (0 through 4)
            return min(i, 4)
    # Fallback: weakest band
    return 0


# Builds the input features and the answer key the AI needs to learn from
# live flux history, so it can predict what's ABOUT to happen next
def engineer_live_features(df):
    """
    Turns the raw live flux readings into a small machine-learning
    dataset:
      - INPUT features: the current flux, a smoothed rolling average,
        how fast it's changing, and the last 2 readings (lag values)
      - ANSWER KEY (label): did the reading two steps ahead end up in a
        HIGHER flare class than right now? (1 = yes, escalating soon;
        0 = no, staying flat or calming down)
    This is what lets the model predict escalation risk, not just
    report the current class using a fixed formula.
    """
    # Makes a safe copy, sorted oldest-to-newest, with a clean row index
    df = df.copy().sort_values("time_tag").reset_index(drop=True)
    # A smoothed 3-point rolling average of the flux (reduces noise)
    df["rolling_mean"] = df["flux"].rolling(window=3, min_periods=1).mean()
    # How much the flux changed compared to the previous reading
    df["rate_of_change"] = df["flux"].diff().fillna(0)
    # The flux value 1 step ago (fills the very first row with itself)
    df["lag1"] = df["flux"].shift(1).fillna(df["flux"].iloc[0])
    # The flux value 2 steps ago
    df["lag2"] = df["flux"].shift(2).fillna(df["flux"].iloc[0])
    # Converts every row's flux into its flare-band number (0-4)
    df["band_now"] = df["flux"].apply(flare_band_index)
    # Looks 2 readings into the future and grabs that future band number
    df["band_future"] = df["band_now"].shift(-2)
    # The answer key: 1 if the future band is higher than the current band
    df["escalation_label"] = (df["band_future"] > df["band_now"]).astype(int)
    # Drops the last 2 rows, since they have no real "future" to compare to
    df = df.iloc[:-2]
    # Sends back the enriched table, ready for training
    return df


# Trains a small AI model that predicts escalation risk from the live feed
def train_live_predictor(df):
    """
    Trains a lightweight Random Forest on the live flux history to
    predict: "is the Sun's activity about to escalate into a stronger
    flare class soon?" Uses a chronological split (train on the
    earlier part of the window, test on the more recent part) since
    this is time-series data - the model must only ever learn from
    the past to guess the future, never the other way around.
    """
    # The columns the model is allowed to look at when predicting
    feature_cols = ["flux", "rolling_mean", "rate_of_change", "lag1", "lag2"]
    # Splits the enriched data into inputs (X) and the answer key (y)
    X_live = df[feature_cols]
    y_live = df["escalation_label"]

    # Marks the boundary between the older 80% (train) and newer 20% (test)
    split = int(len(df) * 0.8)
    # Everything before the split is for training
    X_train, X_test = X_live.iloc[:split], X_live.iloc[split:]
    y_train, y_test = y_live.iloc[:split], y_live.iloc[split:]

    # If the training portion never actually saw an escalation happen...
    if y_train.nunique() < 2:
        # There's nothing meaningful to learn yet - skip training this round
        return None, feature_cols, None

    # Creates a small, fast Random Forest (100 trees, shallow depth)
    model = RandomForestClassifier(
        n_estimators=100, max_depth=5,
        class_weight="balanced", random_state=RANDOM_SEED
    )
    # Trains it on the older portion of the live window
    model.fit(X_train, y_train)

    # Default: we don't know the accuracy yet
    test_accuracy = None
    # Only measure accuracy if the more-recent test portion has real examples of both outcomes
    if len(X_test) > 0 and y_test.nunique() >= 1:
        # Compares the model's guesses on the newer data to what actually happened
        test_accuracy = accuracy_score(y_test, model.predict(X_test))

    # Sends back the trained model, which columns it expects, and its accuracy (if known)
    return model, feature_cols, test_accuracy
...
def update_live_flare_plot(frame, ax, fig):
    ...
    Each time fresh data arrives, it also RETRAINS a small AI model
    (Random Forest) on the live window to predict whether the Sun's
    activity is about to escalate into a stronger flare class soon -
    this prediction is shown directly in the chart title, alongside
    the current class from NOAA's fixed formula.
    """
    ...
    if need_fetch:
        ...
        # ---- Retrain the AI predictor on the freshly fetched window ----
        # Builds the engineered features + escalation answer key from the new data
        enriched = engineer_live_features(_LIVE_STATE["data"])
        # Trains a brand new model on this updated window (same cadence as the data itself)
        model, feature_cols, accuracy = train_live_predictor(enriched)
        # Stores the trained model (or None, if there wasn't enough variety to learn from yet)
        _LIVE_STATE["model"] = model
        # Stores which columns that model expects as input
        _LIVE_STATE["feature_cols"] = feature_cols
        # Stores how accurate the model was on its own held-out recent data
        _LIVE_STATE["model_accuracy"] = accuracy
        # Keeps the enriched, feature-engineered table too, so we can predict on it below
        _LIVE_STATE["enriched"] = enriched
    ...
    # ---- Ask the AI model for its live escalation-risk prediction ----
    # Starts out assuming we have no prediction to show
    escalation_risk = None
    # Only try to predict if we actually have a trained model right now
    if _LIVE_STATE["model"] is not None:
        # Grabs the most recent engineered-feature row (the "right now" snapshot)
        latest_features = _LIVE_STATE["enriched"][_LIVE_STATE["feature_cols"]].iloc[[-1]]
        # Asks the model: what's the probability of escalating to a stronger class soon?
        escalation_risk = _LIVE_STATE["model"].predict_proba(latest_features)[0, 1]
    # Saves it back to the shared cache in case other code wants to read it later
    _LIVE_STATE["escalation_risk"] = escalation_risk
    ...
    # Builds a short text describing what the AI model currently predicts
    if _LIVE_STATE["escalation_risk"] is not None:
        # Turns the 0-1 probability into a friendly percentage string
        ai_line = f"AI-predicted escalation risk: {_LIVE_STATE['escalation_risk']*100:.1f}%"
    else:
        # If there's not enough variety in the data yet to train on
        ai_line = "AI-predicted escalation risk: gathering data..."
    ...
    # Builds and sets the chart's title, showing the current class, AI prediction, and timestamps
    ax.set_title(
        f"{title_prefix}FlareBreak Live Solar Flare Monitor\n"
        f"Current class: {flare_class}   |   {ai_line}\n"
        f"Data time: {data['time_tag'].iloc[-1].strftime('%Y-%m-%d %H:%M UTC')}"
    ...
    if need_fetch:
        # Builds a short text showing the model's own self-measured accuracy, if known
        acc_text = (f"{_LIVE_STATE['model_accuracy']*100:.1f}%"
                    if _LIVE_STATE["model_accuracy"] is not None else "n/a")
        # Prints the latest reading, its flare class, and the AI's prediction to the console
        print(f"[{checked_at}] Latest flux: {latest_y:.2e} W/m\u00b2  ->  Class: {flare_class}  "
              f"|  AI escalation risk: {ai_line.split(': ')[-1]}  |  Model self-test accuracy: {acc_text}")
